...
It uses an [hotspot=getSystemInfo file=0]`@Name` on one of its input parameters.
...
It's setup similarly to the `system` request, with the [hotspot=loadData file=0]`loadData` function
used for the nested `SystemLoadData` object.

The [hotspot=operatingSystemHeader hotspot=javaHeader file=0]`@Name` appears again here.
In this context alongside the [hotspot=operatingSystemHeader hotspot=javaHeader file=0]`@Source` annotation,
it's used to connect the [hotspot=javaFunction file=0]`java` and [hotspot=os file=0]`operatingSystem`
object types to `system` requests and the `system` object type.

Operations of the `mutation` type are be used to edit data.
