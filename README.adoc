...
The [hotspot file=1]`SystemClient` interface is annotated with the [hotspot=clientApi file=1]`@GraphQlClientApi` annotation.
...
If any of the resolvers have input variables, these can be passed in using 
...
// file 2
SystemResource.java
[source, javva, linenums, role='code_column hide_tags=copyright']
----
...
Access the http://localhost:9081/system/properties[^] endpoint to use the `system` resolver through the GraphQL client.
