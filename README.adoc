...
Object types are defined using various annotations added to Java classes, their properties, and their methods. 
...
Setter functions are supported, but are not used in this example application, 
...
We will implement resolvers for `query` operations and `mutation` operation types.
