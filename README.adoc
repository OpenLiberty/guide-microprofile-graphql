...
These are defined using various annotations added to Java classes, their properties, and their methods. 
...
This object type will include the [hotspot file=1]`OperatingSystem` and [hotspot file=0]`JavaInfo` objects as nested objects.
However, these object types are not included as properties here.
...
Finally, adding the [hotspot=graphql file=1]`mpGraphQL` feature to the [hotspot file=1]`server.xml` enables the use of the GraphQL feature in OpenLiberty.
