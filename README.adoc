...
NOTE: This section is for design review purposes and will be removed after the reivew.
Please review the title, description, SEO metadata, and the following outlines.

=== Proposals
1. A single guide for GraphQL and GraphQL client. No "frontend" to try the service out. Testing the service using SmallRye
GraphQL client.
...
This branch contains the summaries for proposal one.

=== What you'll learn

- You'll learn how to build and test a simple microservice with MicroProfile GraphQL, which stores and retrieves data about
...
- `mvn liberty:run`
- Curl command to send a POST request with some queries to the GraphQL endpoint
- `mvn liberty:stop`
...
- Curl requests for sending GraphQL queries for each method
- Explain error handling in detail: compare sending multiple `getArtist()` queries in one request vs one `getArtists()` query

This section will be a lot easier to see if we used the frontend

=== Build the GraphQL client

- Replace `pom.xml` to include dependencies for the GraphQL client
- Explain that GraphQL client requires a template interface
    - Each property in the return type of the interface method is requested in the corresponding query
    - This means that we need a new model class to request `albumCount`

==== Creating additional model classes

- Create the `ArtistWithAlbumCount` class
- Explain the changes
    - New `albumCount` property
    - Why we need `@NotNull`

==== Creating the template interface

- Create the `ArtistServiceAPI` interface
- Explain the changes
    - Describe the queries and mutation methods and the annotations
    - Reiterate why there's two methods for the same query with different return types

=== Testing the application

==== Creating the test class

- Create the `ArtistServiceIT` test class
- Explain how to use the `ArtistServiceAPI` client interface to test the API

==== Running the tests

- Hit enter to run integration tests in dev mode
...
=== What is GraphQL
GraphQL is an open-source data query and manipulation language for APIs, and a runtime for fulfilling these queries.
...
to being defined by endpoints like REST APIs. This enables GraphQL APIs to operate on multiple related resources in a
single query whereas REST APIs would require loading the data from multiple URLs through multiple round-trips to the
server.

You will create a GraphQL microservice that stores, queries and manipulates album and artist information.
...
to assist in development and testing of the API.
...
The MicroProfile GraphQL feature enables the GraphQL runtime and use of GraphQL API for your application.
This feature is already enabled in the [hotspot=graphQLFeature]`server.xml` configuration file.
