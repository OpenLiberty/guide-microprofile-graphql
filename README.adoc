...
Each request's body will determine what kind of operation will be performed as well as its parameters.
...
Each type of operation is capable of accepting input variables through the `POST` request's body.
...
We will add a GraphQL endpoint at the `\http://localhost:9080/graphql` URL. 
...
Try it out by executing the following `curl` command:
...
GraphQL offers an alternate way to access the same application and data.
