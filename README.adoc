...
Press `CTRL+enter` to execute your `query` operation. 
...
The [hotspot file=0]`JavaInfo` class is annotated with an [hotspot=type file=0]`@Type` annotation. 
The [hotspot=type file=0]`@Type` annotation maps this class to define the `java` object type in GraphQL.
...
The [hotspot file=1]`OperatingSystem.java` file is set up similarly.
It maps to the `operatingSystem` object type.
This will describe the operating system's information. 
