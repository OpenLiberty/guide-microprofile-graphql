...
Navigate to the `backend` folder and run Open Liberty in development mode, also known as dev mode.
...
deploys your updates whenever you save a new change.
...
You started the `backend` service in dev mode, so all the changes were automatically picked up. 
