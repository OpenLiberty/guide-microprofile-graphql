...
Compared to REST, GraphQL services require less data fetching resulting in lower applications loading times and lower data transfer costs, 
while allowing clients to better customize requests to the server.
...
You will define object types `java`, `systemMetrics`, and `systemInfo` by creating and applying annotations to the `JavaInfo`, `SystemMetrics`, and `SystemInfo` classes respectively. 
