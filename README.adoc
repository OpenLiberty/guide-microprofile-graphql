...
CRUD operations and their details are differentiated by the contents of the request.
If the operation returns data, the user will also specify what properties of the data that they want returned. 
For read operations, a JSON object is returned that contains only the data and properties specified.
For other operations, a JSON object may be returned containing information such as a success message. 
...
You'll write tests using JUnit to call the application server directly. 
