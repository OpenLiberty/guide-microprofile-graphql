...
In GraphiQL you only have to type the body of the `POST` request, which can simplify manual tests. 
You are also able to view a history of your previous requests and repeat any of them.

In GraphiQL you will also be able to view the application's documentation by clicking the `Docs` button. 
The documentation will describe the GraphQL schema.
This gives details on the various operations available as well as descriptions of them. 
If any data types are mentioned, you will also be able to see descriptions of them and the fields contained within them if applicable. 
...
You will receive a response containing the boolean `true` to notify you that the request was successfully processed.
...
Here is it used for the [hotspot=javaFunction file=3]`JavaInfo` and [hotspot=os file=3]`OperatingSystem` object types. 
