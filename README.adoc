...
:page-description: Learn how to add a GraphQL endpoint to an application using MicroProfile GraphQL and Open Liberty.
...
:page-seo-description: Learn how to add a GraphQL endpoint and use the GraphQL query language as implemented using Microprofile GraphQL
...
You will learn how to add a GraphQL endpoint to an application using Microprofile GraphQL.

GraphQL is an open source data query language. 
It provides another method to interact with your application. 
Unlike REST APIs, each `POST` request that is sent to a GraphQL service goes to a single HTTP endpoint.
Each request's body will determine what kind of operation will be performed and its parameters. 
The kinds of operations this guide will cover are `query` and `mutation` operations.
...
Each type of operation accepts input variables through the `POST` request's body.

If any operation returns data, the user must also specify what properties of the data that they want returned in the request body. 
GraphQL will then return only the properties requested.

The returning of only the specified properties is beneficial when you deal 
...
You'll start with a service that responds to `GET` requests at the `\http://localhost:9080/system/properties` URL
with all of your system information. 
We'll add a GraphQL endpoint at the `\http://localhost:9080/graphql` URL. 
...
GraphiQL will be available at the `\http://localhost:9080/graphiql.html` URL. 
...
All examples in the guide will be done using GraphiQL. 
The result will be the same as if you made an HTTP request using a method such as `curl`.
Access the application through GraphiQL at the http://localhost:9080/graphiql.html[^] URL.

Enter the following GraphQL `query` operation into the input box. 
...
The same output can be seen if you make a `POST` request to the `\http://localhost:9080/graphql` URL. 
Try it out by running the following `curl` command:

[role='command']
----
curl -X POST \
     -H "Content-Type: application/json" \
     -d '{ "query": "query { system { username } }" }' \
     http://localhost:9080/graphql
----

Then, access the application's REST endpoint at the http://localhost:9080/system/properties[^] URL.
Look for the property named `user.name`.
Note that it will match the `username` property from the previous GraphQL `query` operation.
The `user.name` property maps to the `username` property in GraphQL. 
Also, note that the JSON response from the GraphQL operation is much smaller, 
containing only the `username` property that was requested.
...
This annotation maps the [hotspot=getSystemInfo file=0]`getSystemInfo()` function to handling `system` requests, which retrieve the system information. 
...
The [hotspot=operatingSystemHeader hotspot=javaHeader file=0]`@Source` annotation is used for 
any properties that are expensive to calculate or search for.
...
You receive a response containing the boolean `true` to inform you that the request was successfully processed.
...
You can also view the note at the http://localhost:9080/system/properties[^] URL. 
Notice that the `note` property is changed here as well.
You can also edit the note through the REST endpoint with a command such as the following:

[role='command']
----
curl -X POST \
     -H "Content-Type: text/plain" \
     -d "Note edited with REST endpoint" \
     http://localhost:9080/system/editNote
----

If you view the `note` again through either the REST endpoint or the GraphQL endpoint, 
you see the change to the `note` is reflected either way.
The same applies whether you change the `note` using GraphQL or the REST endpoint.

GraphQL offers an alternative way to access the same application and data.
However, all requests going to a single endpoint may be more convenient to use. 
...
You can write tests by using JUnit to call the application server directly using GraphQL.
...
Then, it checks the `username` property to ensure it's returned correctly.
