...
Queries that are made through GraphiQL are the same as queries made through HTTP requests. 
...
Then, it makes a query by using the `system` resolver to ensure that the note was saved correctly. 
