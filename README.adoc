...
While none are used here, setter functions are also supported.
...
Notice that [hotspot file=1]`OperatingSystem` and [hotspot file=0]`JavaInfo` don't have properties or getter functions included in [hotspot file=2]`SystemInfo.java`.
...
Here it adds [hotspot=javaFunction file=0]`JavaInfo` and [hotspot=os file=0]`OperatingSystem` object types as properties 
