...
Queries made through GraphiQL are the same as queries made through HTTP requests. 

In GraphiQL, you can view the application documentation by clicking `Docs`. 
The documentation describes the GraphQL schema.
The schema gives details on the various operations available. 
...
The [hotspot=testGet file=0]`testGetSystem()` method makes a query by using the `system` resolver through a `POST` request.
...
It tries to set the note to be the current system time. 
Then, it makes a query by using the `system` resolver to ensure the note was saved correctly. 
